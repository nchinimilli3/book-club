...
export async function loadWorkspace(env: Env, session: AuthSession | null, clubId: string): Promise<Response> {
  ...
  const [clubResult, membersResult, booksResult, meetingsResult, checkpointsResult, progressResult, questionsResult, checkinsResult] = await env.DB.batch([
    ...
    env.DB.prepare(`SELECT id, title, author, cover_url, description, pages, published_year, isbn, subjects_json, status, start_date, target_finish_date, total_chapters, total_pages, created_by, created_at, updated_at
      FROM books WHERE club_id=? ORDER BY created_at DESC LIMIT 250`).bind(clubId),
  ...
  const currentBook = books.find(book => book.status === 'current') ?? null;
  const currentBookId = typeof currentBook?.id === 'string' ? currentBook.id : null;
  const [postsResult, optionsResult, ratingsResult] = await env.DB.batch([
  ...
  return json({
    club, members, books, currentBook, archiveBooks, archiveBookCount: books.filter(book => book.status === 'completed' || book.status === 'archived').length,
    ...
    myClubRating: ratingsResult.results[0] ?? null, meetingQuestions: questionsResult.results, acquired: 0, lockedPostCount: postsResult.results.filter(row => Boolean((row as Record<string, unknown>).locked)).length,
  });
}
