...
import { cloudApi, cloudRequest } from './cloudApi';
...
export async function getProfile(userId: string): Promise<Profile> {
  ...
  return { id: user.id || userId, displayName: user.name || 'Reader', username: result.settings?.username || undefined, avatarUrl: user.image || undefined, style: result.settings?.style || undefined };
}
...
export async function getSharedMemberProfile(clubId: string, memberId: string) { const result: any = await cloudApi.memberProfile(clubId, memberId); const member = result.member || {}; return { profile: { id: member.id, displayName: member.name || 'Reader', username: member.username || undefined, avatarUrl: member.image || undefined, style: member.style || undefined }, books: (result.library || []).map(libraryItem) }; }
...
export async function previewGoodreadsImport(file: File): Promise<GoodreadsImportPreview> { if (!/\.csv$/i.test(file.name)) throw new Error('Choose a Goodreads CSV file.'); return summary(goodreads(csv(await file.text()))); }
export async function importGoodreads(_userId: string, file: File, onProgress?: (done: number, total: number) => void): Promise<GoodreadsImportResult> { const rows = goodreads(csv(await file.text())); const result: any = await cloudApi.importLibrary(rows.map(row => ({ title: row.title, author: row.author, isbn: row.isbn13 || row.isbn, pages: row.pages, publishedYear: row.year, shelf: row.shelf, rating: row.rating, dateFinished: row.dateRead, source: 'goodreads' }))); rows.forEach((_row, index) => onProgress?.(index + 1, rows.length)); return { ...summary(rows), imported: Number(result.imported || rows.length) }; }
