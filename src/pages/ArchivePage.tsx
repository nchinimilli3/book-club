...
export function ArchivePage({clubId}:{clubId:string}){
  ...
  const yearRatings=yearItems.flatMap(x=>x.ratings||[]);
  function openBook(x:ArchiveItem){sessionStorage.setItem('bookclub:search-return',JSON.stringify({path:`/clubs/${clubId}/archive`,label:'Club shelf',scrollY:window.scrollY}));sessionStorage.setItem('bookclub:open-book',JSON.stringify({key:`db:${x.book.id}`,source:'openlibrary',title:x.book.title,author:x.book.author,cover:x.book.coverUrl||'',pages:x.book.pages}));navigate('/search')}
  ...
  return <div className={`page archive-page tone-${club?.tone||'rose'}`}>
    ...
    {loading?<div className="archive-loading">Opening the shelf…</div>:error?<div className="error-text">{error}</div>:items.length?<section className="archive-grid">{items.map((x,index)=>{
      ...
      return <article key={x.id} className="archive-book"><div className="archive-book-number">{String(index+1).padStart(2,'0')}</div><button type="button" className="archive-book-cover" onClick={()=>openBook(x)} aria-label={`Open ${x.book.title}`}><BookCover title={x.book.title} author={x.book.author} src={x.book.coverUrl}/></button><div className="archive-book-copy"><h2>{x.book.title}</h2><p>{x.book.author}</p>{avg>0&&<div className="archive-rating"><Star fill="currentColor"/> {avg.toFixed(1)}</div>}{rs.filter(r=>r.review).slice(0,3).map((r,i)=><blockquote key={i}>“{r.review}” <span>— {r.displayName}</span></blockquote>)}</div></article>
    })}</section>:<div className="empty-state"><h2>Your finished books will live here.</h2></div>}
  </div>
}
