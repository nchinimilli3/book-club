...
export function ClubsPage(){
  ...
  return <div className="page clubs-page">
    ...
    <section className="club-index">{a.clubs.map(c=><button className={`club-index-row tone-${c.tone}`} key={c.id} onClick={()=>nav(`/clubs/${c.id}`)}><div className="club-color"/><div><h2>{c.name}</h2><p>{c.phase.replaceAll('_',' ')}</p></div><ArrowRight/></button>)}{!a.clubs.length&&<div className="empty-state"><h2>No clubs yet.</h2><p>Start one or join with an invite from a friend.</p></div>}</section>
    <div className="two-actions"><button className="secondary" onClick={()=>setMode('join')}>Join with invite</button></div>
