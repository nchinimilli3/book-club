...
export function NotificationsPage(){
  ...
  return <div className="page notifications-page">
    ...
    <section className="notification-list">{items.length?items.map(n=>{const club=a.clubs.find(c=>c.id===n.clubId);const isBook=Boolean(n.bookCoverUrl);return <button type="button" key={n.id} className={`notification-item ${n.readAt?'is-read':'is-unread'}${isBook?' has-book-cover':''}`} onClick={()=>void open(n)}><span className="notification-status" aria-hidden="true"/><span className={`notification-icon${isBook?' notification-book-cover':''}`}>{n.bookCoverUrl?<img src={n.bookCoverUrl} alt=""/>:<Bell/>}</span><span className="notification-copy"><span className="notification-line"><b>{n.title}</b><time>{ago(n.createdAt)}</time></span>{club&&<span className="notification-club">{club.name}</span>}{n.body&&<span className="notification-body">{n.body}</span>}</span></button>}):<div className="empty-state"><h2>Nothing new.</h2><p>Votes, replies, and meeting changes will show up here.</p></div>}</section>
  </div>
}
