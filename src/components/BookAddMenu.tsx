...
export function BookAddMenu({title,clubName,showClub,busy,savedTargets={},onChoose,onClose}:{title:string;clubName?:string;showClub:boolean;busy:boolean;savedTargets?:Partial<Record<BookAddTarget,boolean>>;onChoose:(target:BookAddTarget)=>void;onClose:()=>void}){
  return <div className="quick-add-menu" role="dialog" aria-modal="false" aria-label={`Add ${title}`}>
    ...
    {showClub&&<button type="button" className={savedTargets.club?'saved':''} disabled={busy} onClick={()=>onChoose('club')}><span>{clubName||'Club'}’s list</span>{savedTargets.club&&<Check/>}</button>}
