...
begin
  ...
  if cid is null then
    select c.id into cid from public.clubs c where lower(c.invite_code)=code and c.archived_at is null limit 1;
  end if;
  if cid is null then raise exception 'Invite not found or no longer valid'; end if;
  insert into public.club_members(club_id,user_id,role) values(cid,auth.uid(),'member') on conflict(club_id,user_id) do nothing;
  insert into public.user_preferences(user_id,active_club_id,updated_at) values(auth.uid(),cid,now()) on conflict(user_id) do update set active_club_id=excluded.active_club_id,updated_at=now();
...
revoke all on public.clubs,public.club_members,public.club_books,public.ballots,public.nominations,public.votes,public.book_checkins,public.reading_progress,public.reading_checkpoints,public.posts,public.replies,public.reactions,public.meetings,public.meeting_rsvps,public.meeting_questions,public.personal_books,public.private_notes,public.saved_quotes,public.book_ratings,public.club_archives,public.notifications,public.club_invites,public.product_events,public.client_errors,public.calendar_connections,public.calendar_event_links from anon;

-- Realtime surfaces that power the multi-user experience.
do $$
declare t text;
begin
  foreach t in array array['posts','replies','reactions','reading_progress','meeting_rsvps','meetings','meeting_questions','club_books','book_ratings','ballots','notifications'] loop
    if not exists(select 1 from pg_publication_tables where pubname='supabase_realtime' and schemaname='public' and tablename=t) then
      execute format('alter publication supabase_realtime add table public.%I',t);
    end if;
  end loop;
...
'save_club_meeting(uuid,uuid,uuid,timestamptz,text,text)','cancel_club_meeting(uuid)','set_meeting_rsvp(uuid,text)',
'finish_club_book(uuid)','save_club_book_rating(uuid,numeric,text,boolean)','archive_club_book(uuid)',
'save_my_profile_style_v3(jsonb)','delete_my_account()','get_locked_post_count(uuid)','get_club_taste_profile(uuid)',
'track_product_event(text,jsonb,uuid)','log_client_error(text,text,jsonb)'];
begin
  ...
  select count(*) into n from (values('posts'),('replies'),('reactions'),('reading_progress'),('meeting_rsvps'),('meetings'),('meeting_questions'),('club_books'),('book_ratings'),('ballots'),('notifications')) t(name) where not exists(select 1 from pg_publication_tables p where p.pubname='supabase_realtime' and p.schemaname='public' and p.tablename=t.name);
